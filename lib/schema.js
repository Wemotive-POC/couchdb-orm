class Schema {
    constructor(schema, options={}) {
        ...
        this.ddocs = {};
        Object.entries(options.ddocs || {})
            .map(([name, ddoc]) => this.addDDoc(name, ddoc));
    }

    addDDoc(name, designDoc) {
        const ddoc = {...designDoc};

        if (designDoc.views) {
            ddoc.views = {};
            Object.entries(designDoc.views)
                .forEach(([viewName, view]) => {
                    const newView = {};
                    newView.map = view.map.toString();
                    if (view.reduce)
                        newView.reduce = view.reduce.toString();
                    ddoc.views[viewName] = {...view, ...newView};
                });
        }

        if (designDoc.updates) {
            ddoc.updates = {};
            Object.entries(designDoc.updates)
                .forEach(([updateName, update]) => {
                    ddoc.updates[updateName] = update.toString();
                });
        }

        if (designDoc.filters) {
            ddoc.filters = {};
            Object.entries(designDoc.filters)
                .forEach(([filterName, filter]) => {
                    ddoc.update[filterName] = filter.toString();
                });
        }

        if (designDoc.validate_doc_update) {
            ddoc.validate_doc_update = designDoc.validate_doc_update.toString();
        }

        this.ddocs[name] = ddoc;
    }
